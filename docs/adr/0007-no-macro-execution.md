...
Word 95 shipped WordBasic. DOCX carries VBA in `vbaProject.bin` and `vbaData.xml`. Macros are the single largest document-as-code attack surface in the office-suite ecosystem. Many users expect we will _not_ execute macros; enterprise IT admins depend on that guarantee.
...
- **Refuse to open `.docm` entirely.** Rejected: users must be able to _read_ macro-bearing documents.
