/**
 * Toggle bulleted list on the selection paragraphs.
 * Uses numId=1 for default bulleted list, ilvl=0.
 * If all selected paragraphs already have numId=1, clears numbering.
 */
...
/**
 * Build a props patch that clears the `numbering` field.
 * exactOptionalPropertyTypes prevents `{ numbering: undefined }` as a literal,
 * so we construct the object and assign the undefined value programmatically.
 */
...
export function createToggleBulletedListCommand(): Command<void> {
  return {
    meta: {
      id: asCommandId('app.format.list.bulleted'),
      title: 'Bulleted List',
      category: 'format',
      coalesceKey: 'format:list:bulleted',
    },
    ...
    run(ctx: CommandContext): Result<Patch, CommandError> {
      ...
      const allBulleted = idArray.every((id) => {
        const num = getParaNumbering(ctx.doc, id);
        return num !== undefined && num.numId === BULLETED_NUM_ID;
      });
      ...
      const ops: Op[] = idArray.map((paragraphId) => ({
        ...
        props: allBulleted
          ? clearNumberingProps()
          : { numbering: { numId: BULLETED_NUM_ID, ilvl: 0 } },
      }));
